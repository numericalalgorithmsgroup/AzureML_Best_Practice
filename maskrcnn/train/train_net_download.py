...
def configure_nccl_settings_from_env():
    ...
    set_environment_variables_for_nccl_backend(
        local_size == global_size, master_port=6105
    )
...
def mlperf_test_early_exit(
    ...
    if iteration > 0 and iteration % iters_per_epoch == 0:
        ...
        model.train()
        dllogger.log(
            step=(iteration, epoch,), data={"BBOX_mAP": bbox_map, "MASK_mAP": segm_map}
        )
...
def test_model(cfg, model, distributed, iters_per_epoch, dllogger):
    ...
    if is_main_process():
        ...
        segm_map = map_results.results["segm"]["AP"]
        dllogger.log(
            step=(cfg.SOLVER.MAX_ITER, cfg.SOLVER.MAX_ITER / iters_per_epoch,),
...
def main():
    ...
    parser.add_argument(
        "--json-summary", help="Out file for DLLogger", default="dllogger.out", type=str,
    )
    parser.add_argument(
        "opts",
    ...
    if args.local_rank == 0:
        ...
        dllogger.log(step="DATASET DOWNLOAD", data={"complete": False})
        coco2017 = Dataset.get_by_name(workspace, args.dataset)
        coco2017.download("/data")
        dllogger.log(step="DATASET DOWNLOAD", data={"complete": True})
